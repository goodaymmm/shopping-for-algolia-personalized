...
Respond only with valid JSON.
  ...
  static getMockAnalysis(query?: string): ImageAnalysis {
    const mockCategories = [
      {
        ...
        occasion: 'casual',
        searchKeywords: ['jacket', 'casual', 'blue', 'cotton', 'everyday']
      },
      {
        ...
        occasion: 'work',
        searchKeywords: ['headphones', 'wireless', 'audio', 'black', 'technology']
      },
      {
        category: 'home',
        style: 'modern',
        ...
        occasion: 'home',
        searchKeywords: ['lamp', 'lighting', 'modern', 'white', 'decor']
      }
    ];

    // Simple logic to select mock category based on query
    let selectedCategory = mockCategories[0];
    if (query) {
      const lowerQuery = query.toLowerCase();
      if (lowerQuery.includes('electronics') || lowerQuery.includes('tech') || lowerQuery.includes('headphones')) {
        selectedCategory = mockCategories[1];
      } else if (lowerQuery.includes('home') || lowerQuery.includes('decor') || lowerQuery.includes('lamp')) {
        selectedCategory = mockCategories[2];
      }
    }
    ...
    return {
      ...
      priceRange: 'mid-range',
      confidence: 0.9,
      description: `Mock analysis for ${selectedCategory.category} product`
    };
  }
}